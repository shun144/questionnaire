...
const GenericScore = () => {
  ...
  const { currentQuestionnarie, isLoading, answerHistories, reset } = useQuestionnaireStore();
    ...
    <>
      ...
      {(currentQuestionnarie.category === 'result') && (
        <div className='flex flex-col justify-start items-center h-full w-full'>
          ...
          <div>{(currentQuestionnarie as ResultType).result}</div>
          {answerHistories.map(x => (
            <div key={x.id}>{x.question} → {x.answer}</div>
          ))}
...
export default memo(GenericScore);
