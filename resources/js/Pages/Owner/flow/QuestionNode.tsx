...
const QuestionNode = ({
    ...
    const handleUpdateChoice = useCallback(
        (event: ChangeEvent<HTMLInputElement>, _choice: ChoiceType) => {
    ...
    const handleDeleteChoice = useCallback(
        (_choice: ChoiceType) => {
            ...
            // 選択肢のコネクション削除
            setEdges((eds) =>
                eds.filter((ed) => ed.sourceHandle != _choice.id)
            );
        },
        [updateNodeData, nodeId, nodeData]
    );
                    ...
                    {nodeData.choices.map((choice, idx) => (
                        <div
                            ...
                            className={`w-full relative ${
                                idx != nodeData.choices.length - 1 && "pb-6"
                            }`}
                        >
